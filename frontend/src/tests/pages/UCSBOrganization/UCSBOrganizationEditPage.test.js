...
describe("UCSBOrganizationEditPage tests", () => {
  ...
  describe("when the backend doesn't return data", () => {
    beforeEach(() => {
      ...
      axiosMock
        .onGet("/api/ucsborganizations", { params: { orgCode: "kasa" } })
        .timeout();
    });
  ...
  describe("tests where backend is working normally", () => {
    beforeEach(() => {
      ...
      axiosMock.onPut("/api/ucsborganizations").reply(200, {
        orgCode: "kasa",
        orgTranslationShort: "Updated KASA",
        orgTranslation: "Updated UCSB Korean American Student Association",
        inactive: true,
      });
    });
    ...
    test("Changes when you click Update", async () => {
      ...
      fireEvent.change(orgTranslationShortField, {
        target: { value: "Updated KASA" },
      });
      fireEvent.change(orgTranslationField, {
        target: { value: "Updated UCSB Korean American Student Association" },
      });
      ...
      expect(mockToast).toHaveBeenCalledWith(
        "UCSB Organization Updated - orgCode: kasa",
      );
      expect(mockNavigate).toHaveBeenCalledWith({ to: "/ucsborganization" });

      expect(axiosMock.history.put.length).toBe(1); // times called
      expect(axiosMock.history.put[0].params).toEqual({ orgCode: "kasa" });
